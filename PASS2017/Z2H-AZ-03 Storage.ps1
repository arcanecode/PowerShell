...
# This just prevents me from hitting F5 and running all during a demo
throw "Don't hit F5 you big dummy."
...
Get-AzureRMStorageAccount -ResourceGroupName PassDev
...
Remove-AzureRmStorageAccount -ResourceGroupName 'PassDev' `
                             -Name 'acstoretest1' `
                             -Force
...
# Now create the RG we'll use, adding a check to prevent creating a
# resource group that already exists
$location = 'southcentralus'
$resourceGroupName = 'PassDev'
...
New-AzureRmStorageAccount -ResourceGroupName 'PassDev' `
                          -Name 'acpstest1' `
                          -Location 'southcentralus' `
                          -Type Standard_LRS

# Confirm it is there
Get-AzureRMStorageAccount -ResourceGroupName PassDev | Format-Table
...
# Or, you can remove the entire resource group
Remove-AzureRmResourceGroup -Name 'PassDev' -Force
Get-AzureRmResourceGroup | Format-Table
